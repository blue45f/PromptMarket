...
4. **도메인 단위 확장** - 기능이 커질 때 `domains/<domain>` 안에 query key, query hook, mutation hook, schema/store를 함께 배치합니다.
   - 웹 앱은 `app`(조립) → `domains`(기능) → `infrastructure`(API 클라이언트) → `shared`(components/hooks/utils/store/i18n/types) 4계층으로 나뉘며, ESLint `boundaries`로 의존 방향을 강제합니다(app→전부, domains→domains/shared/infrastructure, infrastructure→shared/infrastructure, shared→shared).
...
├── domains/         도메인 모듈 (계층: domains)
│   └── marketplace/ query hooks, query keys
├── hooks/           커스텀 React 훅 (계층: shared)
├── pages/           라우트 단위 페이지 모듈 (계층: app)
├── router/          Data Router route object 정의 (계층: app)
├── infrastructure/  API 클라이언트 및 외부 서비스 연동 (계층: infrastructure)
├── store/           Zustand store
├── test/            Vitest setup
├── types/           프론트엔드 공유 타입
├── utils/           순수 유틸리티
...
- `infrastructure/api.ts`는 Axios 인스턴스, 인증 헤더 주입, 에러 메시지 추출을 담당합니다.
- `domains/marketplace/queries.ts`는 리스팅, 구매, 리뷰, 내 계정, 프로필 관련 Query/Mutation hook을 제공합니다.
- `domains/marketplace/queryKeys.ts`는 query key를 readonly tuple로 중앙화합니다.
