...
> = {
  PROMPT:         { label: 'Prompt',         emoji: '✨', gradient: 'from-blue-400 to-indigo-600',     pill: 'bg-blue-50 text-blue-700 ring-blue-200' },
  CLAUDE_MD:      { label: 'CLAUDE.md',      emoji: '📘', gradient: 'from-purple-400 to-fuchsia-600',  pill: 'bg-purple-50 text-purple-700 ring-purple-200' },
  AGENT_MD:       { label: 'agent.md',       emoji: '🤖', gradient: 'from-emerald-400 to-teal-600',    pill: 'bg-emerald-50 text-emerald-700 ring-emerald-200' },
  SKILL:          { label: 'Skill',          emoji: '🧩', gradient: 'from-orange-400 to-amber-600',    pill: 'bg-orange-50 text-orange-700 ring-orange-200' },
  MCP_SERVER:     { label: 'MCP Server',     emoji: '🔌', gradient: 'from-cyan-400 to-sky-600',        pill: 'bg-cyan-50 text-cyan-700 ring-cyan-200' },
  SLASH_COMMAND:  { label: 'Slash Command',  emoji: '⚡', gradient: 'from-yellow-400 to-orange-500',   pill: 'bg-yellow-50 text-yellow-700 ring-yellow-200' },
  SUBAGENT:       { label: 'Sub-agent',      emoji: '🛠', gradient: 'from-pink-400 to-rose-600',       pill: 'bg-pink-50 text-pink-700 ring-pink-200' },
  CURSOR_RULES:   { label: '.cursorrules',   emoji: '🧱', gradient: 'from-slate-500 to-zinc-700',      pill: 'bg-slate-100 text-slate-700 ring-slate-200' },
};
...
export const MODEL_BY_SLUG: Record<string, { label: string; vendor: string; family: string }> =
  Object.fromEntries(MODELS.map((m) => [m.slug, { label: m.label, vendor: m.vendor, family: m.family }]));
...
export const TECHNIQUE_META: Record<PromptTechnique, { label: string; hint: string }> = {
  ...
  'chain-of-thought':  { label: 'Chain-of-Thought',  hint: 'Forces the model to reason step-by-step.' },
  'tree-of-thoughts':  { label: 'Tree-of-Thoughts',  hint: 'Branches multiple reasoning paths and picks the best.' },
  'role-prompt':       { label: 'Role / Persona',    hint: 'Adopts a specific expert persona before answering.' },
  'self-consistency':  { label: 'Self-consistency',  hint: 'Samples multiple CoT runs and majority-votes the answer.' },
...
export const CreateListingSchema = z.object({
  ...
  version:     z.string().regex(/^\d+\.\d+\.\d+$/, 'semver').default('1.0.0'),
