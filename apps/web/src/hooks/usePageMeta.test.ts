...
describe('usePageMeta', () => {
  it('sets document.title on mount and restores it on unmount', () => {
    ...
    const { unmount } = renderHook(() =>
      usePageMeta({ title: '리스팅 · PromptMarket' }),
    );
  ...
  it('upgrades twitter:card when og:image is supplied', () => {
    renderHook(() =>
      usePageMeta({
        title: 't',
  ...
  it('writes canonical link when canonical is set', () => {
    ...
    expect(linkHref('link[rel="canonical"]')).toBe(
      'https://promptmarket.dev/listings/foo',
    );
  });

  it('falls back to the project default twitter:card without og:image', () => {
