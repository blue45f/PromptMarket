...
describe('<AudienceMatch />', () => {
  it('renders fit bullets from the type bucket alone', () => {
    ...
    expect(
      screen.getByText('특정 작업에 맞는 검증된 프롬프트를 바로 쓰고 싶은 분'),
    ).toBeTruthy();
  ...
  it('appends a difficulty fit + mismatch when difficulty is set', () => {
    render(
      <AudienceMatch type="PROMPT" category="Coding" difficulty="beginner" />,
    );
    expect(screen.getByText('입문자도 바로 적용하기 쉬워요')).toBeTruthy();
    expect(
      screen.getByText(/고급 옵션·세팅을 기대하는 분께는 가벼울 수 있어요/),
    ).toBeTruthy();
  });
  ...
  it('explains the technique when one is set', () => {
    ...
    expect(
      screen.getByText(/문제 분해와 단계별 추론을 유도하는 패턴/),
    ).toBeTruthy();
  });
  ...
  it('summarises single-model listings as exclusive', () => {
    ...
    expect(
      screen.getByText('claude-opus-4-7 전용으로 다듬어졌어요'),
    ).toBeTruthy();
  });
  ...
  it('summarises multi-model listings with a count', () => {
    ...
    expect(
      screen.getByText('3개 모델에 최적화 — 비교 실험에 좋아요'),
    ).toBeTruthy();
  });
