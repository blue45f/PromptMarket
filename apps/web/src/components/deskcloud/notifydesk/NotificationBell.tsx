/* eslint-disable react-hooks/refs -- vendored DeskCloud widget */

/**
 * NotifyDesk — 단일 파일 벤더링 컴포넌트 (의존성: react 만).
 * ──────────────────────────────────────────────────────────────────────────
 * npm publish 가 막힌 동안 형제 앱에 그대로 복붙해서 쓰는 버전입니다. 업스트림은
 * @notifydesk/widget 이 react.tsx · client.ts · styles.ts · icons.tsx ·
 * relative-time.ts 로 쪼개져 있는데, 자기-완결(single-file) 벤더링을 위해 필요한
 * 클라이언트 · 스코프 CSS · 아이콘 · 상대시간 유틸을 이 파일 하나에 인라인했습니다.
 ...
 *
 * 백엔드 계약(공개·publishable 키 · Authorization: Bearer pk_…):
 ...
 *
 * 접근성/디자인: role="dialog" 드롭다운 · 포커스 트랩 · Esc · 바깥 클릭 닫기 ·
 * focus-visible · prefers-reduced-motion · 대비 ≥4.5:1 · 그라디언트 텍스트/
 * 글래스모피즘/사이드스트라이프 없음 · 외부 CSS 프레임워크 0(스코프 .nd-* CSS).
 * ──────────────────────────────────────────────────────────────────────────
 */
import {
  useCallback,
...
/* ============================== 상대 시간 =============================== */
/* @notifydesk/widget relative-time 동치. 외부 라이브러리 0(Intl.RelativeTimeFormat). */
