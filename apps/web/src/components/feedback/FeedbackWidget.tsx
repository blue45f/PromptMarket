/**
 * SurveyDesk — vendored feedback widget (dependency: react only).
 * ──────────────────────────────────────────────────────────────────────────
 * Vendored copy of the shared SurveyDesk widget while npm publish is blocked.
 * Self-contained: 0 workspace deps (@surveydesk/shared) — the constants and
 * validation it needs are inlined here. Behaviour/design match @surveydesk/widget.
 *
 * Mounted once in <AppProviders>, gated on VITE_SURVEYDESK_URL so the app is
 * completely unaffected while SurveyDesk is not yet deployed (env unset today):
 ...
 *   GET  {endpoint}/api/surveys/{appId}/active      → SurveyDto (404 if none)
 *   POST {endpoint}/api/surveys/{appId}/responses   → { id, surveyVersion, ... }
 *
 * Accessibility/design: focus-visible · prefers-reduced-motion · contrast ≥4.5:1 ·
 * no gradient text / glassmorphism / side-stripes · 0 external CSS frameworks.
 * Styling stays self-contained (scoped .sd-* CSS) so it never touches the app's
 * Tailwind/OKLCH token system.
 * ──────────────────────────────────────────────────────────────────────────
 */
import {
  useCallback,
...
/** Client-side pre-validation (server re-validates). Mirrors @surveydesk/shared validateAnswers. */
...
// Re-export a few types so consumers can reuse them.
export type { SurveyDto, SurveyQuestion, AnswerValue }
// (note) ReactNode is re-exported to avoid an unused-import warning in some build setups.
