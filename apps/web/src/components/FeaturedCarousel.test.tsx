...
describe('<FeaturedCarousel />', () => {
  it('renders 6 SkeletonCards while loading', () => {
    const { container } = render(
      withProviders(<FeaturedCarousel items={[]} loading />),
    );
  ...
  it('renders one ListingCard per item when not loading', () => {
    render(
      withProviders(
        <FeaturedCarousel items={[card, { ...card, id: 'l2', title: 'Card 2' }]} />,
      ),
    );
  ...
  it('exposes the rail as a region with an accessible label', () => {
    ...
    expect(screen.getByRole('region', { name: 'Featured listings' })).toBeTruthy();
  });
});
