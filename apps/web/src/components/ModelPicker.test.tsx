...
describe('<ModelPicker />', () => {
  it('renders one section per vendor when no query is set', () => {
    ...
    const sectionLabels = screen.getAllByText(
      /^(Anthropic|OpenAI|Google|Meta|Mistral|xAI|DeepSeek|Cohere)$/
    )
  ...
  it('renders a removable chip for each selected slug at the top', () => {
    const onChange = vi.fn()
    render(<ModelPicker value={['claude-opus-4-7']} onChange={onChange} />)
