...
export class ListingsService {
  ...
  private statsCache:
    | { value: { totalListings: number; totalDownloads: number; totalCreators: number }; expires: number }
    | null = null;
  ...
  async list(query: QueryListingsDto) {
    ...
    if (query.model) where.models = { contains: query.model };
    if (query.vendor) {
